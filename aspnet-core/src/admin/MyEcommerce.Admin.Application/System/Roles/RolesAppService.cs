...
namespace MyEcommerce.Admin.Roles
{
        ...
        CreateUpdateRoleDto>, IRolesAppService
    {
        ...
        public async override Task<RoleDto> CreateAsync(CreateUpdateRoleDto input)
        {
            ...
            if (isNameExisted)
            {
                throw new BusinessException(MyEcommerceDomainErrorCodes.RoleNameAlreadyExists).WithData("Name", input.Name!);
            }
        ...
        public async Task<GetPermissionListResultDto> GetPermissionsAsync(string providerName, string providerKey)
        {
            ...
            foreach (var group in groups.Where(x => x.Name.StartsWith("AbpIdentity") || x.Name.StartsWith("MyEcomAdmin")))
            {
                var groupDto = CreatePermissionGroupDto(group);

                var neededCheckPermissions = new List<PermissionDefinition>();

                foreach (var permission in group.GetPermissionsWithChildren().Where(x => x.IsEnabled).Where(x => !x.Providers.Any() || x.Providers.Contains(providerName)))
                {
                ...
                var grantInfoDtos = neededCheckPermissions.Select(CreatePermissionGrantInfoDto).ToList();
