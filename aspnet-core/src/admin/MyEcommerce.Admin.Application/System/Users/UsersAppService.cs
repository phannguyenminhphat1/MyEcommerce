...
namespace MyEcommerce.Admin.Users
{
    public class UsersAppService : CrudAppService<IdentityUser, UserDto, Guid, PagedResultRequestDto, CreateUserDto, UpdateUserDto>, IUsersAppService
    {
        ...
        public async Task<List<UserInListDto>> GetListAllAsync(string filterKeyword)
        {
            ...
            {
                query = query.Where(o => o.Name.ToLower().Contains(filterKeyword) || o.Email.ToLower().Contains(filterKeyword) || o.PhoneNumber.ToLower().Contains(filterKeyword));
            }
        ...
        public async Task<PagedResultDto<UserInListDto>> GetListWithFilterAsync(BaseListFilterDto input)
        {
            ...
            if (!input.Keyword.IsNullOrWhiteSpace())
            {
                ...
                query = query.Where(o => o.Name.ToLower().Contains(input.Keyword) || o.Email.ToLower().Contains(input.Keyword) || o.PhoneNumber.ToLower().Contains(input.Keyword));
            }
        ...
        public async override Task<UserDto> CreateAsync(CreateUserDto input)
        {
            ...
            {
                throw new BusinessException(MyEcommerceDomainErrorCodes.UserNameAlreadyExists);
            }

            var isUserEmailExisted = query.Any(x => x.Email == input.Email);
            if (isUserEmailExisted)
            {
                throw new BusinessException(MyEcommerceDomainErrorCodes.UserEmailAlreadyExists);
            }
